...
## Frontmatter proposto (ao ativar, colar como YAML no topo de `.claude/agents/checkout-asaas.md`)
...
## ⚠ Toda a integração descrita abaixo depende de `feature/supabase-base`

Nada neste arquivo existe em `master`. Antes de qualquer recomendação, este agente deve primeiro confirmar no repositório real (não presumir com base neste texto):

1. Qual é a branch atual (`git branch --show-current`).
2. Se `supabase/functions/create-asaas-checkout`, `supabase/functions/asaas-webhook` e `supabase/migrations/` existem nessa branch (`Glob`/`ls`).
3. Se `@supabase/supabase-js` está em `apps/web/package.json`.
4. Se as páginas de resultado de pedido (`PedidoSucessoPage`, `PedidoPendentePage`, `PedidoErroPage`) existem em `apps/web/src/pages/`.

**Se qualquer um desses itens não existir na branch atual, o agente deve dizer isso explicitamente ("este caminho/arquivo não existe nesta branch") e não inventar conteúdo, comportamento ou estrutura para preencher a lacuna.**
