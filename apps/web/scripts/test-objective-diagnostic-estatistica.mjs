...
const migration = readFileSync(resolve(
  import.meta.dirname,
  '../../../supabase/migrations/20260810185239_expand_objective_diagnostic_estatistica.sql'
), 'utf8');
...
const questions = new Set(
  questionSeed.match(/diagnostico-estatistica-[a-z-]+/g) ?? []
);
const optionRows = optionSeed.match(/\('diagnostico-estatistica-[^']+','[A-D]'/g) ?? [];
const solutionRows = solutionSeed.match(/\('diagnostico-estatistica-[^']+','[A-D]'\)/g) ?? [];
const solutionQuestions = new Set(
  solutionRows.map((row) => row.match(/diagnostico-estatistica-[a-z-]+/)?.[0])
);

assert.deepEqual(questions, expectedQuestions, 'Estatística deve ter uma questão por bloco oficial');
...
assert.match(questionSeed, /'estatistica-406'/);
...
for (const officialReference of [
  /NIST\/SEMATECH e-Handbook of Statistical Methods, seção 1\.3\.3\.7, Box Plot/,
  /NIST\/SEMATECH e-Handbook of Statistical Methods, seção 7\.1\.4, What are confidence intervals/,
  /National Institute of Standards and Technology, Dataplot Linear Fit, modelo Y = A \+ B\*X/,
  /PostgreSQL Documentation, SELECT, GROUP BY e Aggregate Functions/,
]) {
  assert.match(questionSeed, officialReference);
}

assert.equal((questionSeed.match(/item 20\.2\.4\.2\.7/g) ?? []).length, 4);
...
console.log('Estatística: 4 questões, 16 alternativas, vínculos, fontes e proteções aprovados.');
