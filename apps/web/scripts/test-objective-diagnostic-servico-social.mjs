...
const migration = readFileSync(resolve(
  import.meta.dirname,
  '../../../supabase/migrations/20260810204604_expand_objective_diagnostic_servico_social.sql'
), 'utf8');
...
const questions = new Set(
  questionSeed.match(/diagnostico-servico-social-[a-z-]+/g) ?? []
);
const optionRows = optionSeed.match(/\('diagnostico-servico-social-[^']+','[A-D]'/g) ?? [];
const solutionRows = solutionSeed.match(/\('diagnostico-servico-social-[^']+','[A-D]'\)/g) ?? [];
const solutionQuestions = new Set(
  solutionRows.map((row) => row.match(/diagnostico-servico-social-[a-z-]+/)?.[0])
);

assert.deepEqual(questions, expectedQuestions, 'Serviço Social deve ter uma questão por bloco oficial');
...
assert.match(questionSeed, /'servico-social-410'/);
...
for (const officialReference of [
  /Associação Brasileira de Ensino e Pesquisa em Serviço Social, Diretrizes Gerais para o Curso de Serviço Social, 1996, seção 3/,
  /Brasil, Lei nº 8\.662\/1993, art\. 5º, inciso IV/,
  /Conselho Federal de Serviço Social, Resolução CFESS nº 557\/2009, art\. 4º e parágrafos/,
  /Brasil, Constituição da República Federativa do Brasil de 1988, art\. 194/,
]) {
...
assert.equal((questionSeed.match(/item 20\.2\.4\.2\.11/g) ?? []).length, 4);
