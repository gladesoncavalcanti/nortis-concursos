...
const questions = new Set(
  questionSeed.match(/diagnostico-pedagogia-[a-z-]+/g) ?? []
);
const optionRows = optionSeed.match(/\('diagnostico-pedagogia-[^']+','[A-D]'/g) ?? [];
const solutionRows = solutionSeed.match(/\('diagnostico-pedagogia-[^']+','[A-D]'\)/g) ?? [];
const solutionQuestions = new Set(
  solutionRows.map((row) => row.match(/diagnostico-pedagogia-[a-z-]+/)?.[0])
);

assert.deepEqual(questions, expectedQuestions, 'Pedagogia deve ter uma questão por bloco oficial');
...
assert.match(questionSeed, /'pedagogia-408'/);
...
for (const officialReference of [
  /Conselho Nacional de Educação, Resolução CNE\/CP nº 1\/2006, art\. 2º/,
  /Presidência da República, Lei Federal nº 9\.394\/1996, art\. 3º, inciso VIII/,
  /Ministério do Desenvolvimento Social e Combate à Fome e Secretaria Nacional de Assistência Social, Orientações Técnicas sobre o PAIF, volume 2/,
  /Conselho Nacional de Educação, Resolução CNE\/CP nº 1\/2012, arts\. 2º e 3º/,
]) {
...
assert.equal((questionSeed.match(/item 20\.2\.4\.2\.9/g) ?? []).length, 4);
...
console.log('Pedagogia: 4 questões, 16 alternativas, vínculos, fontes e proteções aprovados.');
