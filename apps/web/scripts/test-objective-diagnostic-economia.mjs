...
const migration = readFileSync(resolve(
  import.meta.dirname,
  '../../../supabase/migrations/20260810175956_expand_objective_diagnostic_economia.sql'
), 'utf8');
...
const questions = new Set(
  questionSeed.match(/diagnostico-economia-[a-z-]+/g) ?? []
);
const optionRows = optionSeed.match(/\('diagnostico-economia-[^']+','[A-D]'/g) ?? [];
const solutionRows = solutionSeed.match(/\('diagnostico-economia-[^']+','[A-D]'\)/g) ?? [];
const solutionQuestions = new Set(
  solutionRows.map((row) => row.match(/diagnostico-economia-[a-z-]+/)?.[0])
);

assert.deepEqual(questions, expectedQuestions, 'Economia deve ter uma questão por bloco oficial');
assert.equal(optionRows.length, 12, 'cada questão deve ter quatro alternativas');
assert.equal(solutionRows.length, 3, 'cada questão deve ter uma única solução');
assert.deepEqual(solutionQuestions, expectedQuestions, 'todas as questões devem possuir solução');
assert.equal((questionSeed.match(/Questão autoral Nortis\./g) ?? []).length, 3);
...
assert.match(questionSeed, /'economia-404'/);
...
for (const officialReference of [
  ...
  /Constituição da República Federativa do Brasil de 1988, art\. 155, caput e inciso II/,
  /Instituto Brasileiro de Geografia e Estatística, Indicadores Sociais Mínimos/,
]) {
  assert.match(questionSeed, officialReference);
}

assert.equal((questionSeed.match(/item 20\.2\.4\.2\.5/g) ?? []).length, 3);
...
console.log('Economia: 3 questões, 12 alternativas, vínculos, fontes e proteções aprovados.');
